...
## [1.6.0.14]
- 距離無制限モードでの「話しかけられる距離」を 7y から 15y に拡大
  - HoH 離脱後の配置位置が 7y を超えてループが継続しない問題を修正
  - 通常モードのしきい値は 7y のまま

## [1.6.0.13]
- 距離無制限モードのキュウセイ周回を「手動起点 + 話しかけ距離での自動ループ」に変更
  - 最初の1回はユーザーが手動でキュウセイに話しかけてメニューを開く（起動フラグ `kyuseiLoopStarted`）
  - 以降は、キュウセイに話しかけられる距離（<7y）にいれば、遮蔽物があっても（視線判定 checkLineOfSight=false）自動でインタラクトしてループ
  - 話しかけられる距離から離れると待機に戻る（立ち位置で開始/停止を制御可能）
  - `KyuseiInteractable()` を純粋な距離判定（<7y）に戻した（無制限での「存在のみ」判定をやめた）

## [1.6.0.12]
- 距離無制限時にループ処理が止まる不具合を修正（1.6.0.11 で完全手動になりループしなくなっていた）
  - キュウセイがインタラクト可能（オブジェクトテーブルに存在）なら自動で再入場してループするように復元
  - HoH 離脱後はキュウセイ付近に配置されるため、そのまま自動でループする
  - 手動でメニューを開いた場合も従来どおり続行（隠れた場所で読み込まれていない場合は手動で話しかけて起点とする）
  - 自動での「移動（テレポート・経路探索）」は引き続き行わない。ヘルプの文言を実動作に合わせて微修正

## [1.6.0.11]
- 距離無制限時を「手動キュウセイモード」に変更
  - 有効時はボットが自動でキュウセイへ移動・話しかけることをやめ、ユーザーが手動でキュウセイに話しかけてディープダンジョンのメニューを開いた状態から続行するようにした
  - メニューを検出するとそのまま入場処理（セーブ選択・確定）を行い、HoH 内のホード周回は従来どおり自動。離脱後は再びキュウセイのメニューを待ってループする
  - 待機中はステータスに「キュウセイに話しかけてください」と表示
  - 距離無制限のヘルプに「インタラクトができる隠れた場所でキュウセイに話しかけることでループして動作します」を追記
